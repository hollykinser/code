...
def remove_timepoint_for_position(experiment_root, position, timepoint, dry_run=False):
    ...
    try:
        ...
        print(f'Found entry for removal in position_metadata for position {position}:'+pos_md[offending_timepoint_index]['timepoint'])
        if not dry_run:
...
def remove_dead_timepoints(experiment_root, postmortem_timepoints, dry_run=False):
    ...
    for position, position_annotations in good_annotations.items():
        ...
        for timepoint_info in timepoint_annotations.values():
            if 'stage' in timepoint_info.keys():
                timepoint_stages.append(timepoint_info['stage'])
            else:
                timepoint_stages.append('dead')    
        try:
