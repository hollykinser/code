...
def master_function(measures, data_dirs, save_dir, first_day, last_day, miRNA, fluor_measures=False, auto_measures=False, size_measures=False,control_for=None):
	...
	for i in range(0, len(measures)):
		means, lifespans, worms=get_data(complete_worm_df, measures[i], first_day, last_day)
		all_means.append(means)
	#can supply one measure to control correlations for	
	if control_for:
		control_means, lifespans,worms=get_data(complete_worm_df,control_for,first_day,last_day)	
	else:
		control_means=None
